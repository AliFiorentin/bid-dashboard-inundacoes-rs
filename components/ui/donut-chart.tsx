  ...
  (
    ...
    return (
      <div
        ...
        {...props}
      >
        {/* width/height em % (nao em `size`) -- o tamanho REAL vem do container
            (via `style` acima, que pode ser fixo em px ou fluido com clamp());
            `size`/`strokeWidth` seguem so para a matematica do viewBox/raio,
            que escala proporcionalmente com o SVG independente do px final. */}
        <svg
          width="100%"
          height="100%"
        ...
        {centerContent && (
          <div
            ...
            style={{
              // % (nao px) para acompanhar o tamanho real do container acima,
              // que pode ser fluido (clamp()) -- a proporcao (raio menos a
              // faixa do anel) e' a mesma, so' expressa relativa a `size`.
              width: `${((size - strokeWidth * 2.5) / size) * 100}%`,
              height: `${((size - strokeWidth * 2.5) / size) * 100}%`,
            }}
          >
            {centerContent}
          </div>
